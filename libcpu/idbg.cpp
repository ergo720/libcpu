...
gpr_size_to_type(cpu_t *cpu)
{
	...
	uint32_t size = cpu->info.register_layout[cpu->info.regclass_offset[CPU_REGCLASS_GPR]].bits_size;
...
fpr_size_to_type(cpu_t *cpu)
{
	...
	uint32_t size = cpu->info.register_layout[cpu->info.regclass_offset[CPU_REGCLASS_FPR]].bits_size;
